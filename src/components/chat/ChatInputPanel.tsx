...
export default function ChatInputPanel({
  ...
  const handleSend = useCallback(() => {
    ...
    onSend(trimmed);
  }, [text, backendOk, addToast, t, onSend]);
  ...
  const handleAttach = useCallback(
    (action: 'image' | 'file') => {
      const label = action === 'image' ? '图片上传' : '文件上传';
      addToast(`${label}功能开发中`, 'info');
      dropdown.close();
    },
    [addToast, dropdown],
  );

  // Build the SendOptions payload for the parent to pass downstream.
  // We use selectors directly so the parent doesn't need to re-read them.
  const getSelectedToolIds = useCallback(() => {
    if (selectedTools.length === 0) return undefined;
    const skillSet = new Set((skills || []).map((s) => s.id).filter(Boolean));
    const mcpSet = new Set((mcpServers || []).map((m) => m.id).filter(Boolean));
  ...
  }, [selectedTools, skills, mcpServers]);
  ...
  const placeholder = placeholderProp ?? (sendShortcut === 'ctrl_enter' ? '输入消息... (Ctrl+Enter 发送, Shift+Enter 换行)' : '输入消息... (Enter 发送, Shift+Enter 换行)');
